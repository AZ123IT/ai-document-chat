...
Current limitation: embeddings are still generated by a deterministic
placeholder provider. The local demo validates ingestion, chunk storage,
retrieval plumbing, citations, chat persistence, and optional DeepSeek chat
generation, but it does not validate production-grade semantic relevance yet.
